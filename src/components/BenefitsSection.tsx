...
const BenefitsSection: React.FC<BenefitsSectionProps> = ({ benefits, title, subtitle }) => {
  const cardsRef = useRef<Array<HTMLDivElement | null>>([]);
  const benefitsToRender = benefits && benefits.length ? benefits : defaultBenefits;
  ...
  }, [benefitsToRender.length]);

  return (
    <section className="benefits">
      <div className="container">
        <div className="section-header">
          <h2>{title || 'Todo lo que necesitas para tu viaje'}</h2>
          <p>{subtitle || 'Nuestro servicio está diseñado para ofrecerte la mejor experiencia de conexión, dondequiera que vayas.'}</p>
        </div>
